...
ParameterDouble::ParameterDouble(double prob, std::string n, MultivariateNormalPhylogeneticModel* m, double min):  Parameter(prob, n), lowerBound(min), upperBound(std::numeric_limits<double>::max() / 2), numRejections(0), numAcceptances(0), numAdaptive(10000){
    adaptiveProposalActive = true;
    RandomVariable& rng = RandomVariable::randomVariableInstance();
...
ParameterDouble::ParameterDouble(double prob, std::string n, double min):  Parameter(prob, n), lowerBound(min), upperBound(std::numeric_limits<double>::max() / 2), numRejections(0), numAcceptances(0), numAdaptive(10000){
    adaptiveProposalActive = true;
    RandomVariable& rng = RandomVariable::randomVariableInstance();
    double u = min + Probability::Exponential::rv(&rng, 1) + lowerBound;
    value.push_back(u);

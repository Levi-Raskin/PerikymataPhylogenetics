...
TipModelV2::TipModelV2(std::string tn, Eigen::MatrixXd d, PerikymataHSPv4* m) : PhylogeneticModel(), tipDataIncomplete(d), tipName(tn), model(m), numCols(tipDataIncomplete.cols()), numRows(tipDataIncomplete.rows()), numImputationRejections(0), numImputationAcceptances(0), cachedLnL(0.0), cachedLnP(0.0), lnLDirty(true), lnPDirty(true){
